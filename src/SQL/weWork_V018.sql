...
(
    ...
    GRADE_NAME    VARCHAR2(100)    NULL, 
    CONSTRAINT MANAGER_PK PRIMARY KEY (EMP_NUM)
)
/

COMMENT ON TABLE MANAGER IS '인사관리자'
/

COMMENT ON COLUMN MANAGER.EMP_NUM IS '사원번호PF'
/

COMMENT ON COLUMN MANAGER.EMP_NAME IS '사원명F'
/

COMMENT ON COLUMN MANAGER.DEPT_NAME IS '부서명F'
/

COMMENT ON COLUMN MANAGER.GRADE_NAME IS '직급F'
/

ALTER TABLE MANAGER
    ADD CONSTRAINT FK_MANAGER_EMP_NUM_EMPLOYEE_EM FOREIGN KEY (EMP_NUM)
        REFERENCES EMPLOYEE (EMP_NUM)
/
