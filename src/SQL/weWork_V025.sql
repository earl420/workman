...
ALTER TABLE DEPT
    ADD CONSTRAINT UC_DEPT_NAME UNIQUE (DEPT_NAME)
/
...
COMMENT ON COLUMN GRADE.GRADE_NAME IS '직급명'
/
...
COMMENT ON COLUMN CHAT_ROOM.CHAT_CREATE IS '룸생성일시'
/
...
COMMENT ON COLUMN SCHEDULE.EMP_NUM IS '사원번호F'
/

COMMENT ON COLUMN SCHEDULE.DEPT_NUM IS '부서번호F'
/

COMMENT ON COLUMN SCHEDULE.SCH_EMP_START IS '시작일'
/

COMMENT ON COLUMN SCHEDULE.SCH_EMP_END IS '종료일'
/

COMMENT ON COLUMN SCHEDULE.REGISTER_EMP_NUM IS '등록자'
/

COMMENT ON COLUMN SCHEDULE.SCH_CONTENT IS '내용'
/

COMMENT ON COLUMN SCHEDULE.CTYPE IS 'CTYPE'
/

ALTER TABLE SCHEDULE
    ADD CONSTRAINT FK_SCHEDULE_EMP_NUM_EMPLOYEE_E FOREIGN KEY (EMP_NUM)
        REFERENCES EMPLOYEE (EMP_NUM)
/

ALTER TABLE SCHEDULE
    ADD CONSTRAINT FK_SCHEDULE_DEPT_NUM_DEPT_DEPT FOREIGN KEY (DEPT_NUM)
        REFERENCES DEPT (DEPT_NUM)
/
...
(
    ...
    EXPENSE_TYPE        VARCHAR2(50)      NULL, 
    PARTNER_NUM         VARCHAR2(50)      NULL, 
    EXPENSE_PRICE       INT               NULL, 
...
COMMENT ON COLUMN DOC_EXPENSE.EXPENSE_TYPE IS '지출타입'
/
...
(
    ...
    TIME_ON     VARCHAR2(50)    NULL, 
    TIME_OFF    VARCHAR2(50)    NULL, 
...
(
    DOC_NUM     VARCHAR2(50)    NOT NULL, 
    EMP_NUM1    VARCHAR2(50)    NOT NULL, 
    EMP_NUM2    VARCHAR2(50)    NULL, 
    EMP_NUM3    VARCHAR2(50)    NULL, 
    EMP_NUM4    VARCHAR2(50)    NULL, 
    CONSTRAINT REFERENCE_PK PRIMARY KEY (DOC_NUM)
)
/
...
COMMENT ON COLUMN REFERENCE.EMP_NUM1 IS '참조자1F'
/

COMMENT ON COLUMN REFERENCE.EMP_NUM2 IS '참조자2F'
/

COMMENT ON COLUMN REFERENCE.EMP_NUM3 IS '참조자3F'
/

COMMENT ON COLUMN REFERENCE.EMP_NUM4 IS '참조자4F'
/

ALTER TABLE REFERENCE
    ADD CONSTRAINT FK_REFERENCE_EMP_NUM1_EMPLOYEE FOREIGN KEY (EMP_NUM1)
        REFERENCES EMPLOYEE (EMP_NUM)
/
...
--  승인  순서정렬 뷰
create or replace view ApprovalSort as
select ROWNUM PNUM, B.* from
(
SELECT ROWNUM RNUM, A.* 
FROM (SELECT * FROM Approval ) A
ORDER BY 1 DESC
)B;


-- 문서 전체 리스트 뷰
CREATE OR REPLACE VIEW DOC AS
select DRAFT_NUM AS "DOC_NUM" ,EMP_NUM AS "DOC_EMPNUM", DRAFT_TITLE AS "DOC_TITLE",
        DRAFT_CONTENT AS "DOC_CONTENT" , DRAFT_DATE AS "DOC_DATE", DRAFT_STATUS AS "DOC_STATUS" , CONFIRM_NUM
from DOC_DRAFT
union
select EXPENSE_NUM AS "DOC_NUM" ,EMP_NUM AS "DOC_EMPNUM", EXPENSE_TITLE AS "DOC_TITLE", 
        EXPENSE_CONTENT AS "DOC_CONTENT" , EXPENSE_DATE AS "DOC_DATE", EXPENSE_STAUS AS "DOC_STATUS", CONFIRM_NUM
from DOC_EXPENSE
union
select HOLI_NUM AS "DOC_NUM" ,EMP_NUM AS "DOC_EMPNUM", HOLI_TYPE AS "DOC_TITLE", 
         HOLI_TYPE AS "DOC_CONTENT", HOLI_APPLY AS "DOC_DATE", HOLIDAY_STATUS AS "DOC_STATUS", CONFIRM_NUM
from DOC_HOLIDAY
union
select REQUEST_NUM AS "DOC_NUM" ,EMP_NUM AS "DOC_EMPNUM", REQUEST_TITLE AS "DOC_TITLE", 
         REQUEST_CONTENT AS "DOC_CONTENT", REQUEST_DATE AS "DOC_DATE", REQUEST_STATUS AS "DOC_STATUS", CONFIRM_NUM
from DOC_REQUEST
;
...
INSERT INTO GRADE VALUES(999, 'SYSTEM');


-- 사원 입력
-- 회장
INSERT INTO EMPLOYEE VALUES(NULL, '0901', '강보람', DEFAULT, '여', DEFAULT, DEFAULT, 
                            DEFAULT, 13, 100, 24000000, '20120923', DEFAULT, DEFAULT
                            , DEFAULT, DEFAULT);
-- 사장                            
...
--chatting System
INSERT INTO EMPLOYEE VALUES(NULL,'00','SYSTEM',DEFAULT,'SYS',DEFAULT, DEFAULT, 
                            DEFAULT, 999, 999, 150000000, '20120923', DEFAULT, DEFAULT
                            , DEFAULT, DEFAULT);
...
INSERT INTO EMPLOYEE VALUES(NULL, '0901', '강정학', '20190910@wework.com', '남', '1989/11/10', '010-888-4414', 
                            '전라남도 영광군 염산면 칠산로5길 112,(봉남리)', 8, 302, 80000000, '20131001', DEFAULT, DEFAULT
                            , DEFAULT, DEFAULT);
...
('매출'),200000000, null );
	...
	values 
	(JOURN_NUM_SEQ.nextval,'18/12/31','소모품비',
...
BETWEEN TO_NUMBER(SUBSTR('18/10/01',4,2)) AND TO_NUMBER(SUBSTR('18/12/31',4,2)) and sysdate < add_months(FIXTURE_BUY,12*fixture_endurance+3)),
	'소모품',(select decode(sum(pay_price/fixture_endurance), null, 0, sum(pay_price/fixture_endurance)) from fixture 
	join payment on(fixture_payment=pay_num)
	WHERE EXTRACT(MONTH FROM fixture_buy) 
	BETWEEN TO_NUMBER(SUBSTR('18/10/01',4,2)) AND TO_NUMBER(SUBSTR('18/12/31',4,2)) and sysdate < add_months(FIXTURE_BUY,12*fixture_endurance+3)), null);

	insert into journalizing
	values 
	(JOURN_NUM_SEQ.nextval,'18/12/31',('무형자산상각비'),
    ...
    from license join payment using(pay_num) WHERE EXTRACT(MONTH FROM LICENSE_START) BETWEEN TO_NUMBER(SUBSTR('18/10/01',4,2)) AND TO_NUMBER(SUBSTR('18/12/31',4,2)) and sysdate< add_months(license_end,3)),
('소프트웨어'),
(select decode( sum(pay_price/(extract(year from license_end)-extract(year from LICENSE_START))), null, 0, sum(pay_price/(extract(year from license_end)-extract(year from LICENSE_START)))) 
from license join payment using(pay_num) WHERE EXTRACT(MONTH FROM LICENSE_START) BETWEEN TO_NUMBER(SUBSTR('18/10/01',4,2)) AND TO_NUMBER(SUBSTR('18/12/31',4,2)) and sysdate< add_months(license_end,3)), null);


	insert into journalizing
	values 
	(JOURN_NUM_SEQ.nextval,'18/12/31','매출',
	(select decode(sum(debt_price),null,0,sum(debt_price)) from journalizing
	where debt='급여' 
	and journ_date between '18/10/01' and '18/12/31'),
	'급여',
(select decode(sum(debt_price),null,0,sum(debt_price)) from journalizing
	where debt='급여' 
	and journ_date between '18/10/01' and '18/12/31')
...
insert into journalizing
	values 
	(JOURN_NUM_SEQ.nextval,'18/12/31','세금',
	(select ((select decode(sum(credit_price),null,0,sum(credit_price)) from journalizing where credit='매출' and journ_date between '18/10/01' and '18/12/31')-(select decode(sum(debt_price),null,0,sum(debt_price)) from journalizing join account on(debt=account_name) where ACCOUNT_ALI like '2%' and journ_date between '18/10/01' and '18/12/31'))*0.15 from dual),
	'현금',
(select ((select decode(sum(credit_price),null,0,sum(credit_price)) from journalizing where credit='매출' and journ_date between '18/10/01' and '18/12/31')-(select decode(sum(debt_price),null,0,sum(debt_price)) from journalizing join account on(debt=account_name) where ACCOUNT_ALI like '2%' and journ_date between '18/10/01' and '18/12/31'))*0.15 from dual)
, null);
...
insert into journalizing
	values 
	(JOURN_NUM_SEQ.nextval,'18/12/31','매출',
	(select decode(sum(debt_price),null,0,sum(debt_price)) from journalizing where debt='세금' and journ_date between '18/10/01' and '18/12/31')
	,'세금',
	(select decode(sum(debt_price),null,0,sum(debt_price)) from journalizing where debt='세금' and journ_date between '18/10/01' and '18/12/31')
	, null);
...
('이익잉여금'),((select sum(credit_price) from journalizing where credit='이익잉여금' and journ_date between add_months('18/10/01',-3) and add_months('18/12/31',-3))),
('자본금'),((select sum(credit_price) from journalizing where credit='이익잉여금' and journ_date between add_months('18/10/01',-3) and add_months('18/12/31',-3))), null );


	insert into journalizing
	values 
	(JOURN_NUM_SEQ.nextval,'18/12/31','매출',
	(select ((select decode(sum(credit_price),null,0,sum(credit_price)) from journalizing where credit='매출' and journ_date between '18/10/01' and '18/12/31')-(select decode(sum(debt_price),null,0,sum(debt_price)) from journalizing join account on(debt=account_name) where ACCOUNT_ALI like '2%' and journ_date between '18/10/01' and '18/12/31')) from dual),
	'이익잉여금',
	(select ((select decode(sum(credit_price),null,0,sum(credit_price)) from journalizing where credit='매출' and journ_date between '18/10/01' and '18/12/31')-(select decode(sum(debt_price),null,0,sum(debt_price)) from journalizing join account on(debt=account_name) where ACCOUNT_ALI like '2%' and journ_date between '18/10/01' and '18/12/31')) from dual)
	, null);
...
insert into product values (null, '비싼거', 100000000);
...
-- 출근
-- 인사(강연재)
insert into attendance values('20190004', to_date('2019/10/01', 'yyyy/MM/dd'), to_date('08:40:00', 'HH24:MI:SS'), NULL);
insert into attendance values('20190004', to_date('2019/10/02', 'yyyy/MM/dd'), to_date('11:40:00', 'HH24:MI:SS'), to_date('18:40:00', 'HH24:MI:SS'));
insert into attendance values('20190004', to_date('2019/10/03', 'yyyy/MM/dd'), to_date('13:40:00', 'HH24:MI:SS'), to_date('18:00:00', 'HH24:MI:SS'));
insert into attendance values('20190004', to_date('2019/10/04', 'yyyy/MM/dd'), to_date('07:00:00', 'HH24:MI:SS'), to_date('18:00:00', 'HH24:MI:SS'));
insert into attendance values('20190004', to_date('2019/10/05', 'yyyy/MM/dd'), to_date('08:30:00', 'HH24:MI:SS'), to_date('18:00:00', 'HH24:MI:SS'));
insert into attendance values('20190004', to_date('2019/10/06', 'yyyy/MM/dd'), to_date('08:20:00', 'HH24:MI:SS'), to_date('18:00:00', 'HH24:MI:SS'));
insert into attendance values('20190004', to_date('2019/10/07', 'yyyy/MM/dd'), to_date('08:10:00', 'HH24:MI:SS'), to_date('18:00:00', 'HH24:MI:SS'));
insert into attendance values('20190004', to_date('2019/10/08', 'yyyy/MM/dd'), to_date('08:15:00', 'HH24:MI:SS'), to_date('18:00:00', 'HH24:MI:SS'));
insert into attendance values('20190004', to_date('2019/10/09', 'yyyy/MM/dd'), to_date('08:55:00', 'HH24:MI:SS'), to_date('18:00:00', 'HH24:MI:SS'));
insert into attendance values('20190004', to_date('2019/10/10', 'yyyy/MM/dd'), to_date('08:53:00', 'HH24:MI:SS'), to_date('18:00:00', 'HH24:MI:SS'));
insert into attendance values('20190004', to_date('2019/10/11', 'yyyy/MM/dd'), to_date('08:12:00', 'HH24:MI:SS'), to_date('18:00:00', 'HH24:MI:SS'));
insert into attendance values('20190004', to_date('2019/10/12', 'yyyy/MM/dd'), to_date('08:10:00', 'HH24:MI:SS'), to_date('18:00:00', 'HH24:MI:SS'));
insert into attendance values('20190004', to_date('2019/10/13', 'yyyy/MM/dd'), to_date('09:10:00', 'HH24:MI:SS'), to_date('18:00:00', 'HH24:MI:SS'));
insert into attendance values('20190004', to_date('2019/10/14', 'yyyy/MM/dd'), to_date('10:00:00', 'HH24:MI:SS'), to_date('18:00:00', 'HH24:MI:SS'));
insert into attendance values('20190004', to_date('2019/10/15', 'yyyy/MM/dd'), to_date('11:40:00', 'HH24:MI:SS'), to_date('18:00:00', 'HH24:MI:SS'));
insert into attendance values('20190004', to_date('2019/10/16', 'yyyy/MM/dd'), to_date('09:00:00', 'HH24:MI:SS'), to_date('18:00:00', 'HH24:MI:SS'));
insert into attendance values('20190004', to_date('2019/10/17', 'yyyy/MM/dd'), to_date('09:00:00', 'HH24:MI:SS'), to_date('18:00:00', 'HH24:MI:SS'));
insert into attendance values('20190004', to_date('2019/10/18', 'yyyy/MM/dd'), to_date('08:40:00', 'HH24:MI:SS'), to_date('18:00:00', 'HH24:MI:SS'));
insert into attendance values('20190004', to_date('2019/10/19', 'yyyy/MM/dd'), to_date('08:30:00', 'HH24:MI:SS'), to_date('18:00:00', 'HH24:MI:SS'));
insert into attendance values('20190004', to_date('2019/10/20', 'yyyy/MM/dd'), to_date('08:20:00', 'HH24:MI:SS'), to_date('18:00:00', 'HH24:MI:SS'));
insert into attendance values('20190004', to_date('2019/10/21', 'yyyy/MM/dd'), to_date('08:15:15', 'HH24:MI:SS'), to_date('18:00:00', 'HH24:MI:SS'));
insert into attendance values('20190004', to_date('2019/10/22', 'yyyy/MM/dd'), to_date('08:20:40', 'HH24:MI:SS'), to_date('18:00:00', 'HH24:MI:SS'));
insert into attendance values('20190004', to_date('2019/10/23', 'yyyy/MM/dd'), to_date('08:30:50', 'HH24:MI:SS'), to_date('18:00:00', 'HH24:MI:SS'));
insert into attendance values('20190004', to_date('2019/10/25', 'yyyy/MM/dd'), to_date('08:30:20', 'HH24:MI:SS'), to_date('18:00:00', 'HH24:MI:SS'));
insert into attendance values('20190004', to_date('2019/10/26', 'yyyy/MM/dd'), to_date('08:10:10', 'HH24:MI:SS'), to_date('18:00:00', 'HH24:MI:SS'));
insert into attendance values('20190004', to_date('2019/10/27', 'yyyy/MM/dd'), to_date('08:10:10', 'HH24:MI:SS'), to_date('18:00:00', 'HH24:MI:SS'));
insert into attendance values('20190004', to_date('2019/10/28', 'yyyy/MM/dd'), to_date('08:10:20', 'HH24:MI:SS'), to_date('18:00:00', 'HH24:MI:SS'));
insert into attendance values('20190004', to_date('2019/10/29', 'yyyy/MM/dd'), to_date('08:20:20', 'HH24:MI:SS'), NULL);
insert into attendance values('20190004', to_date('2019/10/30', 'yyyy/MM/dd'), to_date('08:10:20', 'HH24:MI:SS'), to_date('18:00:00', 'HH24:MI:SS'));
insert into attendance values('20190004', to_date('2019/10/31', 'yyyy/MM/dd'), to_date('08:10:10', 'HH24:MI:SS'), to_date('18:00:00', 'HH24:MI:SS'));
-- 총무(양정환)
insert into attendance values('20190022', to_date('2019/10/01', 'yyyy/MM/dd'), to_date('08:40:00', 'HH24:MI:SS'), to_date('18:00:00', 'HH24:MI:SS'));
insert into attendance values('20190022', to_date('2019/10/02', 'yyyy/MM/dd'), to_date('11:40:00', 'HH24:MI:SS'), to_date('18:00:00', 'HH24:MI:SS'));
insert into attendance values('20190022', to_date('2019/10/03', 'yyyy/MM/dd'), to_date('13:40:00', 'HH24:MI:SS'), to_date('18:00:00', 'HH24:MI:SS'));
insert into attendance values('20190022', to_date('2019/10/05', 'yyyy/MM/dd'), to_date('08:30:00', 'HH24:MI:SS'), to_date('13:00:00', 'HH24:MI:SS'));
insert into attendance values('20190022', to_date('2019/10/06', 'yyyy/MM/dd'), to_date('08:20:00', 'HH24:MI:SS'), NULL);
insert into attendance values('20190022', to_date('2019/10/07', 'yyyy/MM/dd'), to_date('08:10:00', 'HH24:MI:SS'), to_date('18:00:00', 'HH24:MI:SS'));
insert into attendance values('20190022', to_date('2019/10/08', 'yyyy/MM/dd'), to_date('08:15:00', 'HH24:MI:SS'), to_date('18:00:00', 'HH24:MI:SS'));
insert into attendance values('20190022', to_date('2019/10/09', 'yyyy/MM/dd'), to_date('08:55:00', 'HH24:MI:SS'), to_date('18:00:00', 'HH24:MI:SS'));
insert into attendance values('20190022', to_date('2019/10/10', 'yyyy/MM/dd'), to_date('08:53:00', 'HH24:MI:SS'), to_date('13:00:00', 'HH24:MI:SS'));
insert into attendance values('20190022', to_date('2019/10/11', 'yyyy/MM/dd'), to_date('08:12:00', 'HH24:MI:SS'), to_date('18:00:00', 'HH24:MI:SS'));
insert into attendance values('20190022', to_date('2019/10/12', 'yyyy/MM/dd'), to_date('08:10:00', 'HH24:MI:SS'), to_date('18:00:00', 'HH24:MI:SS'));
insert into attendance values('20190022', to_date('2019/10/13', 'yyyy/MM/dd'), to_date('09:10:00', 'HH24:MI:SS'), to_date('18:00:00', 'HH24:MI:SS'));
insert into attendance values('20190022', to_date('2019/10/14', 'yyyy/MM/dd'), to_date('10:00:00', 'HH24:MI:SS'), to_date('18:00:00', 'HH24:MI:SS'));
insert into attendance values('20190022', to_date('2019/10/15', 'yyyy/MM/dd'), to_date('11:40:00', 'HH24:MI:SS'), to_date('18:00:00', 'HH24:MI:SS'));
insert into attendance values('20190022', to_date('2019/10/16', 'yyyy/MM/dd'), to_date('09:00:00', 'HH24:MI:SS'), to_date('18:00:00', 'HH24:MI:SS'));
insert into attendance values('20190022', to_date('2019/10/17', 'yyyy/MM/dd'), to_date('09:00:00', 'HH24:MI:SS'), to_date('18:00:00', 'HH24:MI:SS'));
insert into attendance values('20190022', to_date('2019/10/18', 'yyyy/MM/dd'), to_date('08:40:00', 'HH24:MI:SS'), to_date('18:00:00', 'HH24:MI:SS'));
insert into attendance values('20190022', to_date('2019/10/19', 'yyyy/MM/dd'), to_date('08:30:00', 'HH24:MI:SS'), to_date('18:00:00', 'HH24:MI:SS'));
insert into attendance values('20190022', to_date('2019/10/20', 'yyyy/MM/dd'), to_date('08:20:00', 'HH24:MI:SS'), to_date('18:00:00', 'HH24:MI:SS'));
insert into attendance values('20190022', to_date('2019/10/21', 'yyyy/MM/dd'), to_date('08:15:15', 'HH24:MI:SS'), to_date('18:00:00', 'HH24:MI:SS'));
insert into attendance values('20190022', to_date('2019/10/22', 'yyyy/MM/dd'), to_date('08:20:40', 'HH24:MI:SS'), to_date('18:00:00', 'HH24:MI:SS'));
insert into attendance values('20190022', to_date('2019/10/23', 'yyyy/MM/dd'), to_date('08:30:50', 'HH24:MI:SS'), to_date('18:00:00', 'HH24:MI:SS'));
insert into attendance values('20190022', to_date('2019/10/24', 'yyyy/MM/dd'), to_date('08:20:10', 'HH24:MI:SS'), to_date('18:00:00', 'HH24:MI:SS'));
insert into attendance values('20190022', to_date('2019/10/25', 'yyyy/MM/dd'), to_date('08:30:20', 'HH24:MI:SS'), to_date('18:00:00', 'HH24:MI:SS'));
insert into attendance values('20190022', to_date('2019/10/26', 'yyyy/MM/dd'), to_date('08:10:10', 'HH24:MI:SS'), to_date('18:00:00', 'HH24:MI:SS'));
insert into attendance values('20190022', to_date('2019/10/27', 'yyyy/MM/dd'), to_date('08:10:10', 'HH24:MI:SS'), to_date('18:00:00', 'HH24:MI:SS'));
insert into attendance values('20190022', to_date('2019/10/28', 'yyyy/MM/dd'), to_date('08:10:20', 'HH24:MI:SS'), to_date('18:00:00', 'HH24:MI:SS'));
insert into attendance values('20190022', to_date('2019/10/29', 'yyyy/MM/dd'), to_date('08:20:20', 'HH24:MI:SS'), to_date('18:00:00', 'HH24:MI:SS'));
insert into attendance values('20190022', to_date('2019/10/30', 'yyyy/MM/dd'), to_date('08:10:20', 'HH24:MI:SS'), to_date('14:00:00', 'HH24:MI:SS'));
insert into attendance values('20190022', to_date('2019/10/31', 'yyyy/MM/dd'), to_date('08:10:10', 'HH24:MI:SS'), NULL);
